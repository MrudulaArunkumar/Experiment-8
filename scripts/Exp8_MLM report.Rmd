...
**This report highlights the results from the Multi level analysis for Experiment 8.**
...
The prepared dataset is saved as  *Exp8_DataforMLM.csv* and is loaded to perform the MLM.
...
## 1. Model1 - similar to ANOVA data
...
randomSlopes_m1b_err<-glmer(err_trials~1+val*sal + (1|participant), 
                           data=raw.data, 
                           na.action = "na.omit", binomial)
...
## 2. Model2 : Adding Previous Response, Distance and Distance Type as factors
...
randomSlopes_m2_err<-glmer(err_trials~1+val*sal + previous_rm_cwp+ Distance_cwp + Distance_type_cwp +(1+val*sal + previous_rm_cwp + Distance_cwp + Distance_type_cwp|participant), 
                      data=raw.data, 
                      binomial,
                      na.action = "na.omit")
# 
...
pander(anova(randomSlopes_m1, randomSlopes_m4), style = "rmarkdown", caption = "Model 1 vs Model 4, RT")
```

## 3. Model3 : Without the factors of Validity and Saliency but with the interaction between the binding factors of Distance, ResponseRel and Distance Type

There is an effect of Previous Response in RT and a borderline effect in ER. Results not shown since it is not directlt related to overshadowing
...
# 
# 
# tab_model(randomSlopes_m3,randomSlopes_m3_err,show.ci = FALSE, show.stat = TRUE, show.aic = TRUE, show.loglik = TRUE)
# summary(randomSlopes_m3_err)
# 
# summary(randomSlopes_m3)
# #plot_model(randomSlopes_m3, type = "pred", terms = c("previous_rm_cwp","Distance_cwp","Distance_type_cwp"))
```

## 4. Model4 : Adding the interaction of Validity and Saliency to Model 3
...
tab_model(randomSlopes_m4,randomSlopes_m4_err, show.ci = FALSE, show.stat = TRUE, show.aic = TRUE, show.loglik = TRUE)
...
## 5. Model5 : adding the means of Distance and Previous Response also as a factor -- not relevant now

### 1. with RT

```{r m5RT, warning=FALSE, message=FALSE}
...
#                       REML=F,
#                       na.action = "na.omit")
# tab_model(randomSlopes_m5, show.ci = FALSE, show.stat = FALSE, show.aic = TRUE, show.aicc = TRUE, show.loglik = TRUE)
# 
...
Comparing MOdels with only Validity and Saliency (Model 1) vs one where the binding factors of previous response and Distance (Model 2) are introduced. *Error data throws an error*
...
Comparing standard model with model 4 that has the interaction terms of the binding data as well

> Model 4 fits better than Model 1

```{r m1m4, warning=FALSE, message=FALSE}
pander(anova(randomSlopes_m1, randomSlopes_m4), style = "rmarkdown", caption = "Model 1 vs Model 4, RT")
pander(anova(randomSlopes_m1_err, randomSlopes_m4_err), style = "rmarkdown", caption = "Model 1 vs Model 4, RT")
```
### Model 2 vs Model 4
...
pander(anova(randomSlopes_m4, randomSlopes_m2), style = "rmarkdown", caption = "Model 2 vs Model 4, RT")

```


```{r multiplemodel, warning=FALSE, message=FALSE}
#pander(anova(randomSlopes_m1, randomSlopes_m2, randomSlopes_m4,randomSlopes_m5), style = "rmarkdown", caption = "comparing multiple models")
```
> Model 2 is the best fit, that includes the main factors of Previous Response, Distance and Distance Type
 where the main effect of previous response is significant and there is a *almost* significant effect of Validity Saliency interaction.
 
 There is an error with the model comparisons for m2 Error so that needs to be checked further.